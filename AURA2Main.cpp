...
// Define filenames
#define SD_LOG_FILE        			"AURA2.CSV"
#define SD_FLG_FILE        			"CUT1.txt"
...
/* State of the flight */
#define ALT_STEP 20000			//200 Metres in Centimetres	
#define ALT_CUT_ABOVE 200000	//2000 Metres in Centimetres	

long r_altitude = 0; /* Reference altitude */
...
void setup() {
	...
	if(!SD.init(SPI_HALF_SPEED, PIN_SPI_CS)) {
		...
		if (flagFile.exists(SD_FLG_FILE)) {
...
void loop() {
	...
	if(ETin.receiveData(I2C_SLV_SIM900_ADDRESS)){            
		...
		if (i2cIn.balloonCutFired == 1 && mD.vals.balloonCutFired == 0 ) {
			flagFile.open(SD_FLG_FILE, O_CREAT | O_WRITE | O_APPEND);
			char SDString[100] = "";
			int tmp_year = mD.vals.year + 2000;	
	...
	if (NEWGPSDATA) {									//We have new GPS data, get all of the fields we need.
        ...
        if (gps.altitude() != TinyGPS_HJOE::GPS_INVALID_ALTITUDE && gps.altitude() >= 0) {
			...
			byte i = mD.vals.ascent;
			
			if(mD.vals.i_alt >= r_altitude + ALT_STEP)
			{
				/* Payload is rising */
				mD.vals.ascent = 1;
				r_altitude = mD.vals.i_alt;
			}
			else if(mD.vals.i_alt <= r_altitude - ALT_STEP)
			{
				/* Payload is falling */
				mD.vals.ascent = 0;
				r_altitude = mD.vals.i_alt;
			...
			if(mD.vals.i_alt >= ALT_CUT_ABOVE && i == 1 && mD.vals.ascent == 0){
